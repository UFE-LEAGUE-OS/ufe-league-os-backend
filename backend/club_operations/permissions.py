...
class CanAccessClubOperations(BasePermission):
    """
    ...
    - Operations Manager / Commercial Manager via union workspace
    """
    ...
    def has_permission(self, request, view):
        ...
        # Check if user has a club admin scope with operations-relevant role
        ...
        # Check if user is operations/commercial manager in a union workspace
    ...
    def has_object_permission(self, request, view, obj):
        ...
        # Check if user is admin of the object's club
        from accounts.models import Club

        if Club.objects.filter(admin=user, pk=obj_club.pk).exists():
            return True
...
class IsTicketingOfficer(BasePermission):
    """
    Grants ticketing officers access to their own assignments.
    """
    ...
    def has_permission(self, request, view):
        ...
        # Check union workspace ticketing officer role
        if hasattr(user, "union_workspace_memberships"):
            if user.union_workspace_memberships.filter(
                is_active=True,
...
class IsClubAdminOrSuperAdmin(BasePermission):
    def has_permission(self, request, view):
        user = request.user
        if not user or not user.is_authenticated:
            return False
    ...
    def has_object_permission(self, request, view, obj):
        ...
        if user.role == User.Role.SUPER_ADMIN:
            return True
        return getattr(obj, "club_id", None) == getattr(user, "club_id", None)


class IsClubAdminOrSuperAdminReadOnly(BasePermission):
    def has_permission(self, request, view):
        user = request.user
        if not user or not user.is_authenticated:
            return False
